...
import { parseDevis } from "../_shared/_openai.ts";
...
Deno.serve(async (req) => {
  ...
  const roomKey = body.roomKey as string | undefined;
  const documentName = body.documentName as string | undefined;
  if (!projectId || !text) return corsResponse(400, { error: "projectId et text requis." });
  ...
  if (usedImages >= entitlements.limits.ai_images_per_month) {
    return corsResponse(403, {
      error: `Quota mensuel d'analyses IA atteint (${entitlements.limits.ai_images_per_month}/mois pour le plan ${entitlements.planName}). Contacte l'équipe pour en savoir plus.`,
    });
  }

  try {
    const lines = await parseDevis({
      text,
      context: `Devis: ${documentName || "document"}, pièce: ${roomKey || "non précisée"}`,
    });
    ...
    supabaseAdmin.from("ai_usage_events").insert({
      ...
      event_type: "devis_parse",
      input_tokens: 0,
      output_tokens: 0,
      web_search_calls: 0,
    }).then(() => {}).catch(() => {});

    return corsResponse(200, { lines });
  } catch (error) {
