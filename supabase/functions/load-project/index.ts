...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [
      ...
      supabaseUser.from("room_items").select("id, room_key, list_key, text, done, url, image, preview_title, position, due_date, assignee, price, price_currency, selected_for_purchase, status").eq("project_id", id).order("position"),
